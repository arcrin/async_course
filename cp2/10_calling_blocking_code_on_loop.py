...
def multi_block_requests(http, url, n, *, executor=None, loop: asyncio.AbstractEventLoop):
    return (asyncio.ensure_future(block_request(http, url, executor=executor, loop=loop)) for _ in range(n))
...
loop.run_until_complete(consume_responses(*multi_block_requests(HTTP_POOL_MANAGER, URL, 10, loop=loop), loop=loop))
loop.run_until_complete(consume_responses(block_request(HTTP_POOL_MANAGER, 'https://google.ca', loop=loop), loop=loop)) # this will be blocked until 10 requests above have returned
