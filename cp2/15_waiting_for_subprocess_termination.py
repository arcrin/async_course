...
async def is_windows_process_alive(process, delay=0.5):
    """
    On windows the signal API is very sparse, meaning we don't have SIGCHLD. So we just check if we have a return code
    on our process objec.
    :param process:
    :param delay:
    :return:
    """
...
async def main(process_coro, *, loop):
    process = await process_coro  # TODO: what does this await do?
    if sys.platform != 'win32':
